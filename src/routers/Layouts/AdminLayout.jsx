...
export const  AdminLayout = () => {
 const [sidebarCollapsed, setSidebarCollapsed] = useState(() => localStorage.getItem('dashboard-sidebar-collapsed') === 'true');
  ...
  const showEventNav = location.pathname.startsWith('/admin/events');
  const showUserNav = location.pathname.startsWith('/admin/users');
  const showApprovalNav = location.pathname.startsWith('/admin/approvals');
  ...
  const userLinks = [
    { to: '', label: 'All Users', end: true },
    { to: 'students', label: 'Students' },
    { to: 'coordinators', label: 'Coordinators' },
  ];
  const approvalLinks = [
    { to: '', label: 'Pending', end: true },
    ...
    <div className="dashboard-container ">
           ...
           <main className="dashboard-main">
             ...
             <div className="dashboard-content">
              {/* {showEventNav && <EventNavbar basePath="events" links={eventLinks} />} */}
              {showUserNav && <EventNavbar basePath="users" links={userLinks} />}
              {showApprovalNav && <EventNavbar basePath="approvals" links={approvalLinks} />}
              <Outlet />
             </div>
           </main>
         </div>
